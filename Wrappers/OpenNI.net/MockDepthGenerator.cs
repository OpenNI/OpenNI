...
namespace OpenNI
{
	public class MockDepthGenerator : DepthGenerator
	{
		...
		{
            Status.ThrowOnFail(OpenNIImporter.xnMockDepthSetData(this.InternalObject, frameID, timestamp, dataSize, buffer));
        ...
        private static NodeSafeHandle Create(Context context, string name)
		{
            ...
            Status.ThrowOnFail(OpenNIImporter.xnCreateMockNode(context.InternalObject, NodeType.Depth, name, out handle));
